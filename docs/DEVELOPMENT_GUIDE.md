...
### **【進捗：完了】Step 0: コード基盤の整理と安定化 (最優先)**

*   **目的:** コードの保守性・安定性を高めるための基礎固め。
*   **実施内容:**
    *   設定値管理改善（`IS_PRODUCTION` フラグ導入）、関数整理（`Auth.js`, `Utils.js` などへ移動）、バグ修正（タイポ、比較処理）、エラー処理強化を実施。
    *   デバッグログ (`Logger.log`, フロントエンドからの `logToServer`) を多数追加。
    *   ログイン後の画面表示不具合に関するトラブルシューティングを実施し、ファイル名の不一致が原因であることを特定・修正。
*   **状況:** コード基盤整理は完了。ログインおよびアプリ画面の基本表示が正常に動作することを確認。

### **【進捗：これから着手】Step 1: 機能実装と連携テスト**

*   **目的:** 要件定義に基づき、未実装の機能を実装し、連携動作を確認する。
*   **次のアクション:** ログイン後の画面表示問題を解決する。（詳細は後述の課題セクション参照）

### **解決済み: ログイン後の画面表示不具合 (トラブルシューティングログ)**

*   **現象:**
    *   Google認証またはID/PW認証でのログイン処理自体は、バックエンドのログで成功を確認。セッション情報も正しく設定されていた。
    *   しかし、ログイン成功後に表示されるべきアプリ画面 (`app.html`) が表示されず、画面が真っ白になるか、「接続が拒否されました」エラーが表示されていた。
*   **調査と試行錯誤:**
    *   GAS実行ログを詳細に分析した結果、ログイン処理とセッション設定は正常に完了しているが、画面遷移後の `doGet` 関数が URL パラメータ (`?page=app`) や セッションフラグ (`redirectToApp`) を認識できず、常に `index` ページを返そうとしていたことが判明。
    *   以下の対策を試みたが、根本的な解決には至らなかった:
        *   `appsscript.json` の `executeAs` を `USER_ACCESSING` に変更し、スコープ追加と再デプロイ。
        *   スプレッドシートの共有設定確認・修正。
        *   フロントエンドからの画面遷移方法の変更 (`window.location.href`, `replace`, `<a>` タグクリック)。
        *   セッションフラグ (`redirectToApp`) を利用したサーバー側でのページ強制設定。
*   **最終的な原因特定:**
    *   ログイン後に再度 `doGet` が呼び出された際、`page='app'` と判断して `HtmlService.createTemplateFromFile('app')` を実行しようとしたが、GASプロジェクト内に **`app.html` という名前のファイルが存在せず、代わりに `apps.html` というファイル名になっていた**ことが判明。
    *   これにより、テンプレートの読み込みに失敗し、「「app」という HTML ファイルは見つかりませんでした。」というエラーが発生していた（これが真っ白画面や接続拒否の根本原因）。
*   **解決策:**
    *   GASプロジェクト内のファイル名を `apps.html` から 正しい **`app.html`** に修正（または `code.js` 側のファイル名指定を修正）。
    *   修正後、`clasp push` で変更を反映し、`clasp deploy -i <ID>` でデプロイメントを更新。
*   **結果:**
    *   ログイン後、`doGet` が正しく `app.html` を読み込めるようになり、アプリ画面が正常に表示されるようになった。
